...
% Unmix data without AF initialization
% [Y_sps, H_sps] = dataPreprocessing(A0(:,1:end-1),Y);
% H_sps = max(1E6*eps,pinv(A0(:,1:end-1)'*A0(:,1:end-1))*A0(:,1:end-1)'*Y_sps);
% tic; [Anmf, ~] = NMF_RI(Y_sps,A0,H_sps); toc;
% Unmix data without AF initialization - straight line
A0(1:18,end) = 1; A0(19:end,end) = 0; A0(1:end,end) = A0(1:end,end)./sum(A0(1:end,end));
[Y_sps, H_sps] = dataPreprocessing(A0,Y);
tic; [AnmfRI, ~] = NMF_RI_twohier(Y_sps,A0,H_sps); toc;

% % Display the results of each filter
% displayResult_AF_tests(AnmfRI,Anmf_withoutAF,Anmf_withstraightAF,ControlSpectraAll,A0);
%Anmf_withstraightAF
displayResult(AnmfRI,ControlSpectraAll,A0);
%Anmf_withoutAF
% displayResult_AF_tests_easy([Anmf_withoutAF,A0(:,end)],ControlSpectraAll,A0);
% displayResult(Anmf_withoutAF,ControlSpectraAll,A0);
