...
%                 Y_sps = Y_sps(:,1:100:end);
        ...
        if method==1
            if ~exist('RMSEworst','var') 
                RMSEworst = SAD_distance(A, A0);
            else
                RMSEworst = [RMSEworst, SAD_distance(A, A0);];
            end                        
        end
        A0 = A0*diag(1./(sum(A0,1)+eps));
        ...
        % SAD between Href and Hnmf        
        RMSErefNMF(errorCounting) = SAD_distance(Aref, A);
        % SAD between Href and Hfilt
        RMSErefFilt(errorCounting) = SAD_distance(Aref, AFilt);
        % Calculate RACC, Performance evaluation metric.
        RACC(errorCounting) = RMSErefNMF(errorCounting)     
...
%         legend('Reference Spectra','Filter method','Theoretical Spectra','NMF-ML','NMF-SB','NMF-RI','TBCR-NMF','IP-NMF','NMF-ML(Exponential Initialization)','NMF-SB(Gaussian Initialization)','NMF-RI(Exponential Initialization)','TBCR-NMF(nCut Initialization)','IP-NMF(VCA Initialization)');
    ...
    case 'RMSEThRef'
        ...
        [RMSEworstSort,RMSEworstSortIndex] = sort(RMSEworst,'ascend'); % sort element from less to more RMSE.
        figure; 
...
%         plot(VectnChannels/3,RACC_IPNMF,'LineWidth',2,'LineStyle','-','Color',[0.25, 0.25, 0.25]);
        ...
        legend('Reference Spectra','Filter method','Theoretical Spectra','NMF-ML','NMF-SB','NMF-TI','TBCR-NMF');
        axis tight;
